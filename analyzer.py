...
class FloorPlanAnalyzer:
    ...
    def process_file(self, pdf_path: str, save_debug=False):
        ...
        try:
            ...
            # חילוץ קנה מידה - נסה ממספר מקורות
            scale_denom = None

            # 1. אם כבר חולץ scale_text קודם (מ-LLM או manual)
            if meta.get("scale"):
            ...
            # 3. fallback - אם יש scale_value שכבר חושב
            if not scale_denom and "scale_value" in meta:
                # scale_value זה pixels_per_meter, צריך להפוך
                # אבל זה לא אמין, אז נדלג
                pass

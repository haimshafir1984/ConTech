...
class FloorPlanAnalyzer:
    ...
    def _smart_wall_detection(
        ...
        # זיהוי בסיסי — multi-strategy thresholding
        _, otsu = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    ...
    def process_file(self, pdf_path: str, save_debug=False, crop_bbox=None):
        ...
        # הערכה ראשונית של קירות (לצורך Pass 3) — multi-strategy thresholding
        _, _otsu_temp = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
        _adap_temp = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 15, 4)
        binary_temp = cv2.bitwise_or(_otsu_temp, _adap_temp)
        _clean_k_t = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
        binary_temp = cv2.morphologyEx(binary_temp, cv2.MORPH_OPEN, _clean_k_t, iterations=2)
        # הסר קווים דקים (מידות, האצ׳ינג, טקסט) — שמור רק קירות עבים
        _thick_k_t = cv2.getStructuringElement(cv2.MORPH_RECT, (4, 4))
        binary_temp = cv2.morphologyEx(binary_temp, cv2.MORPH_OPEN, _thick_k_t, iterations=2)

...
if mode == "🏢 מנהל פרויקט":
    ...
    with tab1:
        ...
        if st.session_state.projects:
            ...
            current_name = proj["metadata"].get("plan_name", selected.replace(".pdf", ""))
            ...
            with col_edit:
                ...
                if st.button("💾 שמור נתונים", type="primary", use_container_width=True):
                    ...
                    save_plan(
                        selected, p_name, p_scale, scale_val, proj["raw_pixels"],
                        metadata_json, target_date_str, budget_limit_val, cost_per_meter_val,
    ...
    with tab2:
        ...
        if not all_plans:
            st.info("אנא שמור תוכנית אחת לפחות.")
        else:
            ...
            with kpi1:
                st.markdown(f"""<div class="kpi-container"><div class="kpi-icon">🏗️</div><div class="kpi-label">בוצע בפועל</div><div class="kpi-value">{forecast['cumulative_progress']:.1f} מ'</div><div class="kpi-sub">מתוך {forecast['total_planned']:.1f} מ'</div></div>""", unsafe_allow_html=True)
            with kpi2:
            ...
            with kpi3:
                ...
                st.markdown(f"""<div class="kpi-container"><div class="kpi-icon">📅</div><div class="kpi-label">ימים לסיום</div><div class="kpi-value">{days_left}</div><div class="kpi-sub">קצב: {forecast['average_velocity']:.1f} מ'/יום</div></div>""", unsafe_allow_html=True)
            with kpi4:

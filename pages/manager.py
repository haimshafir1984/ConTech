...
from database import (
    save_plan, get_plan_by_id, get_progress_reports,
    get_project_forecast, get_project_financial_status,
    get_payment_invoice_data, get_all_work_types_for_plan,
    get_progress_summary_by_date_range, get_all_plans
)
from utils import (
    safe_process_metadata, safe_analyze_legend,
    load_stats_df, create_colored_overlay
)
            ...
            with st.expander("📐 מדידות מתקדמות (Stage 1+2)", expanded=False):
                ...
                if meta.get("meters_per_pixel"):
                    ...
                    if meta.get("wall_length_total_m"):
                        st.info(f"📏 אורך קירות (skeleton): {meta['wall_length_total_m']:.2f} מטר")
                else:
